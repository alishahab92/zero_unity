...
rosrun zero_sim_ros_b zo_spawn_model.py --spawn_zosim \
...
from zero_sim_ros_b.srv import ZOSimSpawn, ZOSimSpawnRequest, ZOSimSpawnResponse
from zero_sim_ros_b.srv import (
    ZOSimPrefabSpawn,
...
def zo_spawn_prefab_model(
    model_name, model_prefab_name, unity_asset_bundle, initial_pose, zosim_namespace
):
    """[summary]
...
def main():
    ...
    parser.add_argument(
        "--spawn_zosim",
        help="Do a .zosim file based spawn.",
        required=False,
        action="store_true",
    )
    parser.add_argument(
        "--spawn_prefab",
        help="Do a Unity prefab spawn.",
        required=False,
        action="store_true",
    )
    parser.add_argument(
        "--delete_model",
        help="Delete a model in Unity.",
        required=False,
        action="store_true",
    )
    parser.add_argument(
        "--input_zosim_file", help=".zosim file used for input", required=False
    )
    parser.add_argument("--model_name", help="unique name for the model", required=True)
    parser.add_argument(
        "--model_prefab_name", help="Name of the prefab model", required=False
    )
    parser.add_argument(
        "--zosim_namespace",
        help="Namespace of zosim.  Defaults to zerosim",
        default="/zerosim",
        required=False,
    )
    parser.add_argument(
        "--unity_asset_bundle",
        help="The asset bundle to use.  If not specfied then uses the default internal asset bundle.",
        default="",
        required=False,
    )
    ...
    parser.add_argument(
        "--pose_x",
        help="Initial pose X coordinate. 0 is default",
        default=0,
        required=False,
    )
    parser.add_argument(
        "--pose_y",
        help="Initial pose Y coordinate. 0 is default",
        default=0,
        required=False,
    )
    parser.add_argument(
        "--pose_z",
        help="Initial pose Z coordinate. 0 is default",
        default=0,
        required=False,
    )
    parser.add_argument(
        "--pose_roll",
        help="Initial pose roll in degrees. 0 is default",
        default=0,
        required=False,
    )
    parser.add_argument(
        "--pose_pitch",
        help="Initial pose pitch in degrees. 0 is default",
        default=0,
        required=False,
    )
    parser.add_argument(
        "--pose_yaw",
        help="Initial pose yaw in degrees. 0 is default",
        default=0,
        required=False,
    )
